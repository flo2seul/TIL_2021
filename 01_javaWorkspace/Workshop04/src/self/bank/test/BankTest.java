...
public class BankTest {		
	...
	public static void addCustomer() {
		...
		//3. 배열에 추가함..service클래스에 있는 addCustomer() calling
	...
	public static void createAccount() {
		...
		if(BankService.getInstance().getCustomer(ssn).getName()!="") {//회원이 존재한다면
			...
			System.out.println("존재하는 회원이 없습니다..가입부터 시작하세요...");
			return;
		}		
	}

	public static void showCustomer() {//특정회원에 대한 정보를 출력..
		System.out.print("회원의 주민번호를 입력하세요 : ");
		...
		if(BankService.getInstance().getCustomer(ssn).getName()!="") {
			System.out.println(BankService.getInstance().getCustomer(ssn).getInfo());
		}else {
			// 2. 그렇지 않다면 존재하는 회원이 없다는 메세지를 출력하고 return
			System.out.print("존재하는 회원이 없습니다.");
			return;
		}		
	}
